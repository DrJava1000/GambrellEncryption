...
public class VigenereEncryption extends CaesarEncryption // class inherits non-private members and methods from its CaesarEncryption superclass 
{ 
 private String url;  
 ...
 public VigenereEncryption(int caesarShiftAmount) // initialize CaesarEncryption constructor with 7-bit ASCII alphabetic shift count value 
 {
 ...
 public void connectToVigenereCipherDatabase() throws SQLException, ClassNotFoundException 
 {
  ...
  try 
  {
   ...
   System.out.println("Database already exists."); 
	...
	if (e.getSQLState().equals("42X05")) // if CipherConversion table does not exist  
	{
	 tableExists = false; 
	 e.printStackTrace(); 	
	}
 ...
 {                                                    // randomly-chosen decimal values, representing all 7-bit ASCII characters, were mapped 
  ...
  for(int i = 0; i < 128; i++)                        // method generates new conversion table each time program is run if uncommented 
 ...
 public void writeTextFileContents(File outputTextFile, boolean encrypt) throws IOException 
 {
  ...
  if(encrypt) // if true, file is to be encrypted 
  {
   ...
   for(int i = 0; i < inputStrs.size() - 1; i++)
   {
    ...
    try
    { // SQL query is used to select CipherCharacter decimal value in table that corresponds or is mapped to the input plain text character's decimal value  
  ...
  }else
  {
   ...
   for(int i = 0; i < inputStrs.size() - 1; i++)
   {
	...
	try
    { // SQL query is used to select PlainCharacter decimal value in table that corresponds or is mapped to the input cipher text character's decimal value  
