...
public class JavaEncryption extends JFrame {
  ...
  public JavaEncryption(String windowMessage) // constructor makes call to superclass constructor to set window frame message 
  {
   super(windowMessage); 
  }
  ...
  {
   SwingUtilities.invokeLater(new Runnable() // main window is built by event dispatching thread (implemented for good technique)  
		   {
	        public void run()
	        {
	         try
  ...
  { 
   ...
   encryptButton.addActionListener((ActionEvent e) -> { // utilize lambda expression to create anonymous class that implements ActionListener  
   try                                                  // for encrypt button 
   {                                       
   ...
   decryptButton.addActionListener((ActionEvent e) -> { // utilize lambda expression to create anonymous class that implements ActionListener 
   try                                                  // for decrypt button 
   { 
   ...
   caesar.addActionListener((ActionEvent e) -> // utilize lambda expression to create anonymous class that implements ActionListener 
    {
    ...
    vigenere.addActionListener((ActionEvent e) -> // utilize lambda expression to create anonymous class that implements ActionListener for registering Vigenere cipher button  
	 {
  ...
  public void displayFileChooser(JFileChooser fileChooser, boolean encrypt) throws IOException // method retrieves file from a JFileChooser 
  {                                                                                            // if possible and makes calls to encrypt or 
   File selection;                                                                             // decrypt the file as specified 
   ...
   if(encType == 2)
   {
    if(rebuild && check) // if user wants to rebuild, the database values are randomized
	  {
	   try
	   {
  ...
  public File prepareFileWrite(File selectedFile, String oldFileExtension ,String newFileExtension) throws IOException // creates output file for encryption/decryption processes 
  {
  ...
  public static JavaEncryption getMainWindow() // gets main window object 
  {
   return main;  
  }
  
  public void playDialogPopUpAudio(boolean errorMessage) // uses Sound API to play Windows 7 sounds for pop up dialogs 
  {
   try
   {
	File audioFile; 
